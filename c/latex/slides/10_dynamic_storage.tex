...
free(void *ptr);
\end{lstlisting}
	Pass any pointer to previously allocated memory to \textit{free()} and it gets realeased. If you pass pointers on other things, undefined behaviour occurs (most likely programm crashes).
\end{frame}
...
\begin{frame}[fragile]{Reserving large junks}
	...
	\begin{itemize}
		\item Pass the block size $length * sizeof(int)$ to \textit{malloc()}
...
\end{lstlisting}\ \\
	...
	\ \\
	With the help of pointer arithmetic, you can now with the dynamic array like with a "normal" one.
...
\end{lstlisting}
	\begin{itemize}
		\item Allocates a block of $nmemb * size$ bytes, where \textit{nmemb} is supposed the array's length and \textit{size} the size its type.
...
\end{lstlisting}
	\begin{itemize}
		\item \textit{ptr} is pointer to a dynamically allocated memory block
	...
	\begin{itemize}
		...
		\item If it's greater, the block may be at a different location in the memory $\rightarrow$ \textit{ptr} is freed then, also the new area is not initialized
	\end{itemize}
\end{frame}
...
\end{lstlisting}
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item<3-> Hint: always check if you buffer is large enough and resize it, if needed,
		\end{itemize}
		...
		\begin{itemize}
			\item<2-> \textit{strstr()} my be an option.
		\end{itemize}
	\end{itemize}		
\end{frame}
\begin{frame}{Vector operations}
	\begin{itemize}
		\item Write a program that takes two vectors as input.
		\item The number of elements in each vector is upto the user.
		\item Calculate their sum and print the result.
		\begin{itemize}
		...
		\begin{itemize}
			...
			\item<3-> Hint: You need a lot of conventions!
		\end{itemize}
	\end{itemize}
\end{frame}
...
===============
\end{lstlisting}
	\textbf{Experts}: Have a list of words in the source code and let the program randomly choose one at the start.\\
	\ \\
	Since this is the last task, there are no hints. Good luck!
\end{frame}
