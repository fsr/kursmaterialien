...
\begin{frame}[fragile]{Defining functions}
	...
	\end{columns}
	\begin{textblock}{7}(2,3.75)
		\begin{tikzpicture}[x=1.2cm,y=.5cm]
			\uncover<2->{\draw (0,0) edge[out=270,in=90,->,shorten >=0ex] (.5,-2);}
			\uncover<3->{\draw (3.5,0) edge[out=270,in=90,->,shorten >=0ex] (2.5,-2);}
			\uncover<4->{\draw (7,0) edge[out=270,in=90,->,shorten >=.0ex] (4.5,-2);}
		\end{tikzpicture}
	\end{textblock}
	\ \\\ \\\ \\
	\begin{lstlisting}[numbers=none]
...
\end{lstlisting}
	\begin{enumerate}
		\item just as in \textit{main()}, all statements are put in here
		\item value this function returns or empty, if the return value is \textit{void}
	\end{enumerate}
\end{frame}
